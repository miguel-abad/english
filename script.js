...
async function cargarFichas() {
    try {
        ...
        todasLasFichas = await respuesta.json();
        mostrarGaleria();
...
function mostrarGaleria() {
    ...
    if ( fichasFiltradas.length === 0 ) {
        ...
        contador.textContent = "0 fichas encontradas.";
        sinResultados.style.display = "block";
        return;
    }

    sinResultados.style.display = "none";
    contador.textContent = `${fichasFiltradas.length} ficha(s) encontrada(s).`;
    ...
    fichasFiltradas.forEach(
        (ficha, indice) => {
            ...
            tarjeta.innerHTML = `
                ...
                <div class="info">
                    <span class="grado"> ${ficha.grado.toUpperCase()} </span>
...
function actualizarImagen() {
    ...
    const ficha = fichasFiltradas[indiceActual];
    ...
    imagenGrande.src = ficha.ruta;
    imagenGrande.alt = ficha.nombre;
    nombreImagen.textContent = `${ficha.grado.toUpperCase()} — ${ficha.nombre}`;
